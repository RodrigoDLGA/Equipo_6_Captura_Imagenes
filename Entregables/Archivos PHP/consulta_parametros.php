...
/* Observacion : instalar paquete php5-gd para Ubuntu... 	*/
	...
	$consulta="SELECT id_parametro, entrada, salida, intervalo, cantidad from parametros";
	$registros=mysqli_query($conexion,$consulta);
	
	if($registros->num_rows>0){
		while($fila=$registros->fetch_assoc()){
			echo $fila["id_parametro"]."|";
		}
	} else {
	...
	echo "+";
	$consulta="SELECT id_parametro, entrada, salida, intervalo, cantidad from parametros order by id_parametro desc limit 1";
	$registros=mysqli_query($conexion,$consulta);
	
	if($registros->num_rows>0){
		while($fila=$registros->fetch_assoc()){
			echo $fila["entrada"]."|".$fila["intervalo"]."|".$fila["cantidad"];
		}
	} else {
		echo "Error al hacer la consulta a la BD";
	}
